...
- **Stake weight:** affects routing (streams)
- **Stake exposure:** affects risk + reward accounting (conviction)
...
- you cannot “escape” slashing risk by removing weight right before resolution
- exposure is locked **per Goal** (principal can’t exit the Goal until Goal resolution)

> Decision: **conviction continues accruing while exposure is locked**, even if weight is set to 0.
...
Conviction is the time-weighted accumulator used for rewards.

Baseline definition:

- `conviction_u = ∫ exposure_u(t) dt`
