...
The system sets the Goal target outflow rate as:

This means the Goal spends its streamable balance evenly over the remaining time.
...
If this cap binds, the leftover stays in the parent treasury.

**Design note:** Cobuild does **not** automatically redistribute overflow to other children when a cap binds. Overflow remains unspent at the parent until stakers update weights or caps lift.
...
Underwriter exposure remains locked during funding; if the budget expires pre-activation, exposure unlocks for reuse within the Goal and rewards from this budget are zero.
...
**Spend pacing note:** if a budget reaches late execution with funds still unspent, the target rate increases as timeRemaining shrinks. Downstream flows can accelerate near the end of execution.

## Underwriting requirement for child staking
Rule:

> You may only stake into a child mechanism if you also have exposure staked on the parent budget.
> Recommended default: `childExposure_u <= coverageRatio * parentExposure_u` (coverageRatio defaults to `1.0`).

This makes “underwriting inside the budget” conditional on underwriting the budget itself.
